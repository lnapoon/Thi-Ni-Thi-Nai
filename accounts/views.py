...
class ProfileView(View):
    def get(self, request, username=None):
        ...
        bookmarked_checkins = []
        if is_owner:
            bookmarked_ids = Bookmark.objects.filter(user=request.user).values_list('checkin_id', flat=True)
            bookmarked_checkins = CheckIn.objects.filter(id__in=bookmarked_ids).order_by('-created_at')

        # Liked checkins
        liked_checkins = []
        if is_owner:
            liked_ids = Like.objects.filter(user=request.user).values_list('checkin_id', flat=True)
            liked_checkins = CheckIn.objects.filter(id__in=liked_ids).order_by('-created_at')
...
class ToggleFollowView(LoginRequiredMixin, View):
    def post(self, request, username):
        ...
        if target_user == request.user:
            return JsonResponse({'success': False, 'error': 'คุณไม่สามารถติดตามตนเองได้'}, status=400)
        ...
        next_url = request.POST.get('next', reverse('accounts:profile_user', kwargs={'username': username}))
...
class UserFollowingListView(LoginRequiredMixin, View):
    def get(self, request, username):
        target_user = get_object_or_404(User, username=username)
        following_list = Follow.objects.filter(follower=target_user).select_related('following', 'following__profile')
